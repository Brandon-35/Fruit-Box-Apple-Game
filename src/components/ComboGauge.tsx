    ...
    <div className="w-full max-w-[1100px] mb-6">
      <div className="flex justify-between items-center mb-2">
        <div className="flex items-center gap-2">
          <span className={`text-xs font-semibold uppercase tracking-wider transition-colors ${isFeverMode ? 'text-game-accent' : 'text-game-text-muted'}`}>
          ...
          {timeFrozenLeft > 0 && (
            <m.div
              ...
              className="bg-blue-500 text-white text-xs font-bold px-2 py-0.5 rounded-full flex items-center gap-1"
            >
              <Clock size={12} /> {Math.ceil(timeFrozenLeft)}s
            </m.div>
          )}
          {comboCount > 0 && !isFeverMode && (
            <m.span
              initial={shouldReduceMotion ? false : { scale: 0.5, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              className="bg-game-accent text-white text-xs font-bold px-2 py-0.5 rounded-full"
            >
              {comboCount}x
            </m.span>
          )}
      ...
      <div className="h-2.5 w-full bg-game-surface-alt rounded-full overflow-hidden border border-game-border">
        <m.div
          ...
          animate={(isFeverMode && !shouldReduceMotion) ? { backgroundPosition: ['0% 50%', '100% 50%', '0% 50%'] } : {}}
      ...
      <div className="flex justify-between mt-1.5">
        <span className="text-xs font-medium text-game-text-muted uppercase tracking-wider">
